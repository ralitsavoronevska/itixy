$(document).ready(function () {
  ...
  var newCurrentDateTime = moment().add('days', 7);
  ...
  $('#dateRangeMonthPicker').on('change', function () {
    ...
    var newDateRangeEnd = moment(newDateRangeStart).add(7, 'days');
    ...
    currentDateTime = moment(hiddenVal).subtract('days', 7);
    newCurrentDateTime = hiddenVal;
  });

  $('#buttonDateNextWrap').click(function () {
    var newDateVal = moment(hiddenVal).add(7, 'days');
  ...
  $('#buttonDatePreviousWrap').click(function () {
    var startDateTemp = moment(hiddenVal).subtract(14, 'days');
    var endDateTemp = moment(hiddenVal).subtract(7, 'days');
  ...
  $(document).on('click', '.cal-cell[date-id]', function () {
    ...
    $('.selected-date-area').css('display', 'block');
    $('.selected-date-area').find('span')
      .text('From: ' + moment(selectedDate).format('DD-MMM-YYYY'));
  });
...
(function ($, window, undefined) {
  $.fn.rangeCalendar = function (options) {
    ...
    this.each(function (i, el) {
      var obj = el;
      var $el = $(el);
      var settings = $.extend(!0, {}, defaults, options);
      ...
      obj.showCalendar = function (animate) {
        ...
        var startDateFormatted = (startDateIndex >= 0 ? moment()
            .add('days', startDateIndex + obj.start).format() : null);
        var endDateFormatted = (endDateIndex >= 0 ? moment()
            .add('days', endDateIndex + obj.start).format() : null);
        ...
        obj.calendarObj.find('.cell.start').nextAll()
        .slice(0, (!rangeWidth ? obj.minRangeWidth - 1 : rangeWidth - 1)).addClass('selected');
        ...
        var currentCellMonth = obj.calendarObj
            .find('.cell[month-id="' + currentMonthId + '"].selected').eq(0);
        var monthPosition = (!currentCellMonth.length ? obj.calendarObj
            .find('.cell[month-id="' + currentMonthId + '"]')
            .eq(0).position() : currentCellMonth.position());
        ...
        if (rightBar > lastCalItem.index()) {
          obj.calendarObj.find('.cell').eq(currentCalItem.index() - rangeWidth + delta + 1)
          .trigger('click');
          return;
        }
        ...
        $(selectedCell)
        .append('<div class="range-bar resizable"><div class="range-bar-content"></div></div>');
        ...
        obj.calendarObj.draggable({
          ...
          create: function () {
            ...
            obj.calendarObj.find('.cell.start')
                .nextAll().slice(0, obj._rangeWidth - 1).addClass('selected');
          ...
          drag: function (e, ui) {
            ...
            var nextMonthId = parseInt(obj.monthsObj.find('.cell.selected')
                .next().attr('month-id'));
            var prevMonthId = parseInt(obj.monthsObj.find('.cell.selected')
                .prev().attr('month-id'));
            if (nextMonthId && currentMonthId && nextMonthId <= monthMaxId && direction == 'left') {
              ...
              var nextMonthCalendarCell = obj.calendarObj
                  .find('.cell[month-id="' + nextMonthId + '"]').first();
              ...
              var prevMonthCalendarCell = obj.calendarObj
                  .find('.cell[month-id="' + prevMonthId + '"]').last();
          ...
          if (distance > minDistance) {
            ...
            draggableObj.animate({
              ...
              step: function (currentStep) {
                ...
                var newTop = (position.top + (speedY * stepDuration / friction));
                console.log(newLeft);
        ...
        $el.hover(function () {
          if (obj.autoHideMonths) obj.monthsObj.slideDown(100, 'easeOutCirc');
        }, function () {
          if (obj.autoHideMonths) obj.monthsObj.slideUp(0, 'easeOutCirc');
        });
